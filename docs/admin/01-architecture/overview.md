...
flowchart TD
    ...
    subgraph Hosting["ホスティング"]
        GHPages["GitHub Pages<br/>ステージング"]
        Sakura["さくらインターネット<br/>本番"]
    end

    subgraph Workers["Cloudflare Workers"]
        ContentsAPI["miyosino-contents-api"]
        OtherAPIs["各種 API Workers<br/>(announcements / circulars<br/>/ events / minutes 等)"]
    end

    subgraph DataStores["データストア"]
        MicroCMS["MicroCMS<br/>トップ画像 / 四季<br/>自治会活動 / 住民サークル<br/>共用施設 / サービス"]
        Kintone["Kintone<br/>お知らせ / 回覧板<br/>イベント / 会議情報<br/>各種申請 / グリーンウェルネス<br/>周辺施設（#120）"]
    end
...
```mermaid
flowchart LR
    GHA["GitHub Actions<br/>sync.yml<br/>毎週月曜 AM 3:00 JST"]
    Places["Google Cloud<br/>Places API"]
    Script["scripts/<br/>sync-places.ts"]
    Kintone["Kintone<br/>アプリ #120"]
...
> **MicroCMSとKintoneの使い分け:** 公開ページのコンテンツ（画像・施設説明等）はMicroCMS、組合員専用ページの情報（お知らせ・回覧板等）はKintoneで管理しています。詳細は [MicroCMS（コンテンツストア）](microcms.md) を参照してください。
